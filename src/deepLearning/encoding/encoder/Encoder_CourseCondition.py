...
class CourseConditionClass(XClass):
    ...
    def fix(self):
        ...
        if ALGO == 1:
            ...
            condition_dict = {'良':-1, '稍':0, '稍重':1, '重':2, '不':3, '不良':3, '良ダート':4, '稍重ダート':5, '重ダート':6, '不良ダート':7}
            condition_onehot = [0] * len(set(condition_dict.values()))
            ...
            # 馬場状態のone-hot表現
            condition_dict = {'良':0, '稍':1, '稍重':2, '重':3, '不':4, '不良':4, '良ダート':5, '稍重ダート':6, '重ダート':7, '不良ダート':8}
            condition_onehot = [0] * len(set(condition_dict.values()))
            hot_idx = condition_dict[self.xList]
            condition_onehot[hot_idx] = 1
            self.xList = condition_onehot
